...
async function dockerCommand(
  ...
  const { stdout: COMMIT_SHORT_SHA }: { stdout: string } = await utils.exec(
  ...
  const tagList = customTag
    ? [customTag]
    : defaultTagList(image, COMMIT_SHORT_SHA.trim(), imageTagShaTS);
...
async function _push(image: string, tagList: string[], platforms: string[]) {
  ...
  for (const tag of tagList) {
    ...
    await utils.spawn(
      `docker push us-docker.pkg.dev/matterlabs-infra/matterlabs-docker/${image}:${tag}`
    );
    ...
    if (image == "circuit-synthesizer") {
      ...
      await utils.spawn(
        `docker push asia-docker.pkg.dev/matterlabs-infra/matterlabs-docker/${image}:${tag}`
      );
      await utils.spawn(
        `docker push europe-docker.pkg.dev/matterlabs-infra/matterlabs-docker/${image}:${tag}`
      );
    }
  }
}
...
export async function customBuildForHyperchain(
  image: string,
  dockerOrg: string
) {
